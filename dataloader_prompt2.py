...
class EntityEndtokenDataset(torch.utils.data.Dataset):
  """ entity representation 구성을 위한 class."""
  def __init__(self, input_ids, token_type_ids, attention_mask, labels, ss_arr=None, os_arr=None, se_arr=None, oe_arr=None):
  ...
  def __getitem__(self, idx):
    if len(self.labels) == 0:
        return (torch.tensor(self.input_ids[idx]), torch.tensor(self.attention_mask[idx]), torch.tensor(self.token_type_ids[idx]),
                torch.tensor([]), torch.tensor(self.ss_arr[idx]), torch.tensor(self.os_arr[idx]), torch.tensor(self.se_arr[idx]), torch.tensor(self.oe_arr[idx]))
    
    else:

        return (torch.tensor(self.input_ids[idx]), torch.tensor(self.attention_mask[idx]), torch.tensor(self.token_type_ids[idx]),
                torch.tensor(self.labels[idx]), torch.tensor(self.ss_arr[idx]), torch.tensor(self.os_arr[idx]), torch.tensor(self.se_arr[idx]), torch.tensor(self.oe_arr[idx]))
...
class PromptDataloader(pl.LightningDataModule):
    ...
    def tokenizing(self, dataset):
        ...
        for _, item in tqdm(dataset.iterrows(), desc='tokenizing', total=len(dataset)):
            ...
            if self.representation_style == 'typed_marker':
                ...
                if subj_s < obj_s: # subj 가 먼저 나올 때
                    ...
                    start_prompt = f"다음 문장에서 {tmp[1]}와 {tmp[3]}의 관계를 추출하시오. "
                    end_prompt = f" {tmp[1]}와 {tmp[3]}의 관계는 어떤 관계인가?"
                    ...
                    start_prompt = f"다음 문장에서 {tmp[3]}와 {tmp[1]}의 관계를 추출하시오. "
                    end_prompt = f" {tmp[3]}와 {tmp[1]}의 관계는 어떤 관계인가?"
                else:
            ...
            # start prompt 만큼 길이 더해주기
            ss = len(start_prompt) + len(self.tokenizer(tmp[0], add_special_tokens=False)['input_ids']) + 1
            ...
            se = ss + len(self.tokenizer(tmp[1], add_special_tokens=False)['input_ids']) - 1
            oe = os + len(self.tokenizer(tmp[3], add_special_tokens=False)['input_ids']) - 1
        ...
        return input_ids, token_type_ids, attention_mask, ss_arr, os_arr, se_arr, oe_arr
    ...
    def preprocessing(self, data):
        ...
        input_ids, token_type_ids, attention_mask, ss_arr, os_arr, se_arr, oe_arr = self.tokenizing(dataset)

        targets = self.label_to_num(dataset['label'])

        return input_ids, token_type_ids, attention_mask, targets, ss_arr, os_arr, se_arr, oe_arr
    ...
    def setup(self, stage='fit'):
        ...
        if stage == 'fit':
            ...
            train_inputs_ids, train_token_type_ids, train_attention_mask, train_targets, train_ss_arr, train_os_arr, train_se_arr, train_oe_arr = self.preprocessing(train_data)
            val_inputs_ids, val_token_type_ids, val_attention_mask, val_targets, valid_ss_arr, valid_os_arr, valid_se_arr, valid_oe_arr = self.preprocessing(val_data)

            self.train_dataset = EntityEndtokenDataset(train_inputs_ids, train_token_type_ids, train_attention_mask, train_targets, train_ss_arr, train_os_arr, train_se_arr, train_oe_arr)
            self.val_dataset = EntityEndtokenDataset(val_inputs_ids, val_token_type_ids, val_attention_mask, val_targets, valid_ss_arr, valid_os_arr, valid_se_arr, valid_oe_arr)
        else:
            test_data = pd.read_csv(self.dev_path)

            test_inputs_ids, test_token_type_ids, test_attention_mask, test_targets, test_ss_arr, test_os_arr, test_se_arr, test_oe_arr = self.preprocessing(test_data)
            self.test_dataset = EntityEndtokenDataset(test_inputs_ids, test_token_type_ids, test_attention_mask, test_targets, test_ss_arr, test_os_arr, test_se_arr, test_oe_arr)

            predict_data = pd.read_csv(self.predict_path)
            predict_inputs_ids, predict_token_type_ids, predict_attention_mask, predict_targets, predict_ss_arr, predict_os_arr, predict_se_arr, predict_oe_arr = self.preprocessing(predict_data)
            self.predict_dataset = EntityEndtokenDataset(predict_inputs_ids, predict_token_type_ids, predict_attention_mask, predict_targets, predict_ss_arr, predict_os_arr, predict_se_arr, predict_oe_arr)
