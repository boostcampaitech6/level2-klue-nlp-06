...
def main(config):
    ...
    #labels_not_to_augment = label_counts[label_counts > config['augment_target']].index.to_list()
    
    #no_augment_df = total_df[total_df['label'].isin(labels_not_to_augment)]
    #no_augment_df = no_augment_df.reset_index(drop=True)
    
    #to_augment_df = total_df[total_df['label'].isin(labels_to_augment)]
    #to_augment_df = to_augment_df.reset_index(drop=True)
    ...
    integrated_data = pd.concat([new_data, df], ignore_index=True)
    
    if config['train_aug'] == True:
        integrated_data.to_csv('./dataset/'+config['model_name'].split("/")[-1]+'-'+str(config['augment_target'])+'-'+str(config['masking_num'])+'-' + 'train.csv', index=False)
    else:
        integrated_data.to_csv('./dataset/'+config['model_name'].split("/")[-1]+'-'+str(config['augment_target'])+'-'+str(config['masking_num'])+'-' + 'dev.csv', index=False)
